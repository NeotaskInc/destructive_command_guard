...
| Kind | Name | Reason | Regex Preview |
|------|------|--------|---------------|
| destructive | `regex", "reason")` - unnamed, default High severity
...
mod tests {
    ...
    fn matches_safe_with_deadline_returns_false_when_expired() {
        ...
        let pack = Pack {
            ...
            keywords: &["rm"],
            safe_patterns: vec![
                safe_pattern!("bt_safe_1` | Found '(?=' | `(?=.*safe_target)rm\s+--dry-run` |
...
| safe | `gcloud-storage-buckets-list` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+buckets\s+l...` |
| safe | `gcloud-storage-buckets-describe` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+buckets\s+d...` |
| safe | `gcloud-storage-objects-list` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+objects\s+l...` |
| safe | `gcloud-storage-objects-describe` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+objects\s+d...` |
| safe | `gcloud-storage-ls` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+ls(?=\s\|$)` |
| safe | `gcloud-storage-cat` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+cat(?=\s\|$)` |
| safe | `gcloud-storage-cp` | Found '(?=' | `gcloud\b(?:\s+--?\S+(?:\s+\S+)?)*\s+storage\s+cp(?=\s\|$)` |
| destructive | `gsutil-rb` | Found '(?=' | `gsutil\b.*?\brb(?=\s\|$)` |
